...
export default function ProjectsView({ view, setView }: ProjectsViewProps) {
    return (
        ...
        >
            {/* Left Sidebar Index */}
            <div
                className={`w-full shrink-0 md:w-[30%] h-auto md:h-screen md:fixed left-0 border-b md:border-b-0 md:border-r border-[#1F2833] bg-[#0B0C10] p-8 md:p-16 overflow-visible md:overflow-y-auto hide-scrollbar z-10 flex flex-col justify-between smooth-transition delay-100
          ${view === 'projects' ? 'translate-x-0 opacity-100' : '-translate-x-16 opacity-0'}`}
            >
                <div>
                    ...
                    <div className="mb-12">
                        <h3 className="text-[10px] font-black text-[#5C6B73] uppercase tracking-[0.3em] mb-5">By Protocol:</h3>
                        <ul className="space-y-4 font-mono text-[10px] text-[#45A29E]">
                            {['00 Python', '01 SQL', '02 Next.js', '03 TensorFlow', '04 Scikit-Learn'].map((tech, i) => (
                                <li key={i} className="flex hover:text-white cursor-pointer transition-colors group">
                                    <span className="w-10 text-[#5C6B73] group-hover:text-[#45A29E]">/{tech.split(' ')[0]}</span>
                ...
                <div className="hidden md:block">
                    ...
                    <p className="text-[9px] font-mono text-[#5C6B73] uppercase tracking-[0.4em]">© 10-03-2026 {portfolioData.personalInfo.name.toUpperCase()}</p>
                </div>
            </div>

            {/* Right Scrollable Content */}
            <div
                className={`w-full shrink-0 md:w-[70%] md:ml-[30%] h-auto md:h-screen overflow-visible md:overflow-y-auto p-8 md:p-24 smooth-transition delay-150 bg-[#12141D]
          ${view === 'projects' ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0'}`}
            >
                    ...
                    <div key={project.id} className="mb-48 group">
                        <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-8 border-l-2 border-[#1F2833] pl-6 group-hover:border-[#45A29E] transition-colors">
                            <div className="flex flex-col gap-2">
                                <span className="font-mono text-[10px] text-[#5C6B73] tracking-widest uppercase mb-1">PROT_VER: {project.year}</span>
                        ...
                        <div className="w-full aspect-[16/9] bg-[#0B0C10] rounded-none overflow-hidden relative border border-[#1F2833] flex items-center justify-center group-hover:border-[#45A29E]/50 transition-colors">
                            ...
                            <div className="z-10 text-center bg-[#12141D]/90 backdrop-blur-sm p-12 border border-[#1F2833] shadow-2xl">
                                <p className="font-mono text-[10px] text-[#5C6B73] mb-8 uppercase tracking-[0.5em] animate-pulse">[[ NEURAL_RENDER_ACTIVE ]]</p>
                ...
                <div className="h-24 flex flex-col items-center justify-center border-t border-[#1F2833] mt-20 gap-4">
                    ...
                    <p className="font-mono text-[8px] text-[#5C6B73] uppercase tracking-[0.4em]">© 2026 — PROTOCOL_STABLE</p>
                </div>
            </div>
        </div>
    );

...
export default function NotesView({ view, setView }: NotesViewProps) {
    return (
        <div
            className={`absolute inset-0 flex flex-col w-full h-screen text-[#C5C6C7] items-center justify-center p-8 md:p-20 smooth-transition bg-[#0B0C10]
        ...
        >
            ...
            <div className={`w-full max-w-4xl z-10 smooth-transition delay-100 ${view === 'notes' ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
                    ...
                    {portfolioData.notes.length > 0 ? (
                        portfolioData.notes.map(note => (
                            <div key={note.id} className="border-l-2 border-[#1F2833] pl-10 pb-10 group hover:border-[#45A29E] transition-colors relative">
                                ...
                                <span className="text-[10px] font-mono text-[#1F2833] mb-4 block tracking-[0.3em] uppercase">{note.date} // ARCHIVE_PHASE</span>
                        ...
                        <p className="text-xl text-[#1F2833] font-mono uppercase tracking-[0.2em] animate-pulse">[[ DATA_STREAM_EMPTY ]]</p>
                    )}
                </div>
            </div>
        </div>
